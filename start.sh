...
# Check that srcds_run exists
if [ ! -f $GAME_DIR/srcds_run ]; then
    echo "Error: srcds_run not found! Black Mesa did not download correctly."
    exit 1
fi
...
# Make sure curl/wget exits on failure
set -e
